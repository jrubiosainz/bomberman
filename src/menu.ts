...
export class MenuSystem {
  ...
  private isEditingKey: boolean = false;
  private soundEnabled: boolean = true;

  constructor() {
    this.mode = 'main_menu' as GameMode;
    this.apiKey = localStorage.getItem('bomberman_llm_key') || '';
  ...
  private handleLLMSetup(action: InputAction | 'enter' | 'escape' | 'backspace' | string): void {
    if (this.isEditingKey) {
      if (action === 'enter') {
        ...
        localStorage.setItem('bomberman_llm_key', this.apiKey);
      } else if (action === 'escape') {
        this.isEditingKey = false;
        this.apiKeyInput = this.apiKey;
      } else if (action === 'backspace') {
        this.apiKeyInput = this.apiKeyInput.slice(0, -1);
      } else if (typeof action === 'string' && action.length === 1) {
        this.apiKeyInput += action;
      }
    } else {
      if (action === 'up') {
        this.selectedIndex = Math.max(0, this.selectedIndex - 1);
      ...
      } else if (action === 'left' && this.selectedIndex === 2) {
        this.selectedLevel = Math.max(0, this.selectedLevel - 1);
      } else if (action === 'right' && this.selectedIndex === 2) {
        this.selectedLevel = Math.min(LEVEL_CONFIGS.length - 1, this.selectedLevel + 1);
      } else if (action === 'enter') {
  ...
  private renderLLMSetup(ctx: CanvasRenderingContext2D, width: number, height: number, _time: number): void {
    ...
    ctx.fillStyle = '#0ff';
    ctx.font = 'bold 48px Arial';
    ctx.fillText('🤖 LLM PLAYS BOMBERMAN', width / 2, 80);
    ...
    // API Key
    const keyY = startY + spacing;
    ctx.textAlign = 'left';
    ctx.fillStyle = this.selectedIndex === 1 ? '#ffd700' : '#fff';
    ctx.font = 'bold 20px Arial';
    ctx.fillText('API Key:', labelX, keyY);
    
    const displayKey = this.isEditingKey ? this.apiKeyInput : (this.apiKey ? '●●●●●' + this.apiKey.slice(-4) : 'Not set');
    ctx.fillStyle = this.isEditingKey ? '#0f0' : (this.selectedIndex === 1 ? '#0ff' : '#aaa');
    ctx.fillText(this.isEditingKey ? displayKey + '|' : displayKey, valueX, keyY);

    if (this.selectedIndex === 1 && !this.isEditingKey) {
      ctx.fillStyle = '#666';
      ctx.font = '12px Arial';
      ctx.fillText('Press ENTER to edit', valueX, keyY + 25);
    }
    ...
    ctx.fillStyle = this.selectedIndex === 2 ? '#0ff' : '#aaa';
    ctx.fillText(`< Level ${level.level}: ${level.name} >`, valueX, levelY);

    // Start button
    const startY2 = startY + spacing * 3;
    ctx.textAlign = 'center';
    const canStart = !!this.apiKey;
    ctx.fillStyle = this.selectedIndex === 3 ? (canStart ? '#0f0' : '#f00') : (canStart ? '#aaa' : '#555');
    ctx.font = 'bold 24px Arial';
    ctx.fillText(canStart ? '▶ START' : '⚠ API KEY REQUIRED', width / 2, startY2);
